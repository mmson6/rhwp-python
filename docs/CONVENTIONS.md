...
대표 형태 1 종 (Frozen). 나머지 (`Draft` / `Active` / `Superseded`) 는 schema 표의 필드 조합 적용 — 실제 사례는 `docs/roadmap/` / `docs/upstream/` 에서 참조.
...
본문 구조의 룰 SSOT 는 `/new-spec` skill 의 템플릿 파일이 보유:

- **Spec body skeleton**: [`.claude/skills/new-spec/templates/spec.md`](../.claude/skills/new-spec/templates/spec.md) — placeholder 만, 직접 복사 대상
- **Spec body 작성 룰**: [`.claude/skills/new-spec/references/spec.md`](../.claude/skills/new-spec/references/spec.md) — 섹션별 prose (title 단일 phrase, 인트로 핵심 요약만 디테일 제외, 결정 사항 표 형식, AC-N behavior-driven, 영구 비목표 등)
- **ADR skeleton**: [`.claude/skills/new-spec/templates/adr.md`](../.claude/skills/new-spec/templates/adr.md) — placeholder 만
- **ADR 작성 룰**: [`.claude/skills/new-spec/references/adr.md`](../.claude/skills/new-spec/references/adr.md) — 섹션별 prose (인트로 표준 phrase, 결정 매트릭스 표 형식, 4-소절 (팩트 / 검증자 반박 / 최종 결정 / 1차 소스))

수동 작성 / Frozen 후 typo 수정 / 신규 spec 모두 이 두 템플릿이 단일 SSOT. `/new-spec` skill 은 본 템플릿을 복사해 scaffold — drift 위험 없이 자동화 + 수동 동시 적용.

본 § 와 템플릿 파일이 어긋나면 **템플릿 파일이 우선**.

### 적용 시점

본 § + 템플릿 발효일 (2026-05-03) 이후 신규 작성 spec / ADR. 기존 Frozen spec (v0.1.0 ~ v0.3.1) 은 미적용 — Frozen 본문 변경 금지 정책 (immutability) 우선.

## 섹션 역할 분리 — 정보 배치 룩업

"이 정보는 어디 적어야 하나?" 의 단일 SSOT 룩업 표. spec ↔ ADR ↔ CHANGELOG ↔ implementation log 의 역할 비대칭을 명시화하여 *같은 정보가 여러 곳에 중복* 되거나 *어디에도 없어 사라짐* 을 방지.

| 정보 | 위치 |
|---|---|
| spec 의 본 의도 / 영향 / 변경 없음 보장 | spec 인트로 |
| 결정의 raw 디테일 (commit hash / PR # / 일자 / 정책 phrase) | spec 결정 사항 표 셀 |
| 옵션 비교 / 검증자 반박 / 1차 소스 | ADR §N 의 4-소절 |
| 사용자 visible 변화 (*what* — 추가 / 변경 / 제거된 API, extras, 호환성) | CHANGELOG `[X.Y.Z]` 섹션 |
| stage 별 작업 흐름 / 시행착오 / a/b/c 옵션 의사결정 (*why·how*) | implementation log (`docs/implementation/...`) |
| spec ↔ test 매핑 (자동 생성) | `docs/traces/coverage.md` (Living) |
| 외부 시스템에 흘러가기 전 staging | `docs/upstream/<topic>.md` (Active) |

핵심 분리 룰:
- *같은 사실 중복 기록 금지* — CHANGELOG 가 *what*, implementation log 가 *why·how*. 결정 비교 (a/b/c) 가치가 없는 변경 (단순 dep bump, typo) 은 CHANGELOG 한 줄로 충분 — implementation log 작성 안 함
- *spec 인트로에 디테일 침투 금지* — 디테일은 결정 사항 표 셀로
- *spec 결정 사항 표에 옵션 비교 침투 금지* — 옵션 / 대안은 ADR 4-소절로

본 § 는 § CHANGELOG ↔ implementation log 역할 분리 의 확장 — 본 표가 master, 그 § 는 두 행만 다루는 부분 표.
...
Major release GA 시점에 직전 major 의 frozen spec 을 `docs/archive/v<N>/` 로 이동 (동일 구조 유지, 본문 변경 없음). README 인덱스는 active major 만 노출, archive 는 별도 페이지 한 줄 link. 목적은 인덱스 / 검색 가독성 — git size 무관. **v1.0 GA 직전 작업** 이라 그 전까지는 적용 보류.
