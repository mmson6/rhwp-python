...
- **AC-5** — MCP `render_page_png(path, 0)` 도구 호출이 fastmcp `ImageContent` 인스턴스 반환 (속성 `mimeType == "image/png"`, `data` 가 base64-decoded PNG magic 일치). `outputSchema` 에 `ImageContent` `$ref` 노출
