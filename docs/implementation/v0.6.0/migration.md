...
| **CI `test-without-extras` job (skip count 5 → 6)** | `tests/test_render_png.py` 가 file-level `pytest.importorskip("PIL.Image")` 게이트 — Pillow 미설치 환경에서 file 전체가 1 skip 으로 카운트. ci.yml + AGENTS.md 양쪽의 expected skip count 5 → 6 갱신. MCP `TestMcpRenderPagePng` 는 per-test `importorskip("fastmcp")` 라 fastmcp 미설치 시 한 테스트만 skip (file-level 카운트 영향 0) |
